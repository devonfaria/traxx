// Defining variables to select important parts of our HTML
...
// This function handles the submit button of our index.html page
...
// This function handles the submit button of our search-results.html page
...
var fetchTrackID = function (searchTerm) {
  ...
  fetch(apiTrackID).then(function (response) {
    if (response.ok) {
      response.json().then(function (data) {
        for (var i = 0; i < data.length; i++) {
          ...
          resultButtonEl.setAttribute(
            "id",
            `${data[i].track.track_name} - ${data[i].track.artist_name}`
          );
